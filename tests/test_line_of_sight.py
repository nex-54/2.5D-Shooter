...
class LineOfSightTests(unittest.TestCase):
    ...
    def assert_sight_both_directions(self, start: tuple[float, float],
                                     end: tuple[float, float], expected: bool) -> None:
        for source, target in ((start, end), (end, start)):
            with self.subTest(source=source, target=target):
                self.assertEqual(gmap.has_line_of_sight(*source, *target), expected)
    ...
    def test_exact_corner_contacts_do_not_allow_sight_through_walls(self) -> None:
        for tile in (1, gmap.DOOR_TILE, gmap.BARRIER_TILE):
            ...
            with self.subTest(tile=tile):
                self.assert_sight_both_directions((2.5, 3.5), (3.5, 2.5),
                                                 tile == gmap.BARRIER_TILE)
    ...
    def test_wall_corners_block_hitscan_and_enemy_detection(self) -> None:
        ...
        self.assertIsNone(hitscan([enemy], px, py, angle))
        enemy.update(px, py, 0)
