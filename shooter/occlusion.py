...
class DepthBuffer:
    ...
    def __init__(self) -> None:
        self.pixels: NDArray[np.float32] = np.full((WIDTH, HEIGHT), np.inf,
                                                  dtype=np.float32)

    def clear(self) -> None:
        self.pixels.fill(np.inf)

    def block_column(self, x: int, width: int, depth: float,
                     top: int = 0, bottom: int = HEIGHT) -> None:
    ...
    @contextmanager
    def sprite(self, screen: pygame.Surface, bounds: pygame.Rect,
               depth: float) -> Generator[tuple[pygame.Surface, int, int] | None, None, None]:
        ...
        if not bounds:
            yield None
            return
        visible = self.pixels[bounds.left:bounds.right,
                              bounds.top:bounds.bottom] >= depth - 0.1
        if visible.all():
            yield screen, 0, 0
